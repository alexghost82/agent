...
// own skillIds are still surfaced as skill nodes (as in the thin seed) so the
// add-skill flow's anchor/idempotency keeps working.
export function buildDesignMapFromScanGraph(
  ...
  // Skill nodes from project.skillIds, placed beyond the intel columns so they
  // never overlap, and hung off the design anchor (mirrors the thin seed).
  const skillIds = (project.skillIds ?? []).slice(0, MAX_SKILL_NODES);
